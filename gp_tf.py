...
from util import read_wine, read_wine_quality
import matplotlib.pyplot as plt


# Results of GPR.
# contents : y predictions and sigmas.
...
# Results of GPR_GD.
# Inherit GPRResult. // Parent : GPRResult, Child : GPR_GDResult.
# contents : minimum loss and configuration of minimum loss.
...
# Gassian Process Regression model.
class GPR(object):
    ...
    # Check the size X is less than MAX_TRAIN_SIZE or not. 
    ...
    # Predict GPR result.                  
    # return type : GPRResult()            
    # return contents: yhats and sigmas
    ...
    # return contents : leangth scale, magnitude, X_train, y_train, xy_, K, K_inv
    ...
    # Not used.
    # Calculate GPR_UCB parameter beta.

# Gaussian Process Regression Gradient Descent model 
# Inherit GPR class.
class GPR_GD(GPR):
    ...
    DEFAULT_RIDGE = 1.0
    # Parameter for finding new configuration.
    ...
    # Fit GRP_GD model. 
    # super function for inheritance relation between GPR and GPR_GD. 
    def fit(self, X_train, y_train, ridge=DEFAULT_RIDGE):
        ...
        with tf.Session(graph=self.graph, config=tf.ConfigProto(
            ...
            #===========================================#
            # Loss function with GP-UCB (same as paper) #
            #===========================================#
            Loss = tf.squeeze(tf.subtract(yhat_gd, calculate_sigma_multiplier(X_train.shape[0], X_train.shape[1]) * sig_val))
    ...
    # Predict GPR result.                  
    # return type : GPR_GDResult()           
    # return contents: yhats, sigmas, minLs, minL_confs.
    def predict(self, X_test, constraint_helper=None,categorical_feature_method='hillclimbing',categorical_feature_steps=3):
...
# Main function.                                                    
# Run GR_GD with random generated metrices(create_random_matrices())
def main():
    check_gd_equivalence()

# Create random matrices.                       
# n_samples : the number of training samples    
...
def create_random_matrices(n_samples=3000, n_feats=12, n_test=4444):
    ...
    # n = n_samples + n_test
    # X_train = np.random.rand(n, n_feats)
    ...
    #return X_train, y_train, X_test, y_test, length_scale, magnitude, ridge

def wine(n_test=20):
    y_train, X_train = read_wine()
    ...
    length_scale = np.random.rand()
    magnitude = np.random.rand()
    ridge = np.ones(X_train.shape[0]) * np.random.rand()
    return X_train, y_train, X_test, y_test, length_scale, magnitude, ridge

def wine_quality(n_test=20):
    y, X = read_wine_quality()
    X_test = X[:n_test]
    y_test = y[:n_test].reshape(-1,1)
    X_train = X[n_test:]
    y_train = y[n_test:].reshape(-1,1)
...
# Run GPR with random generated matrices.
...
# Run GPR_GD with random generated matrices.
def check_gd_equivalence():
    X_train, y_train, X_test, length_scale, magnitude, ridge = create_random_matrices(n_test=3)
    #X_train, y_train, X_test, y_test, length_scale, magnitude, ridge = create_random_matrices(n_test=3)
    #X_train, y_train, X_test, y_test, length_scale, magnitude, ridge = wine(n_test=5)
    #X_train, y_train, X_test, y_test, length_scale, magnitude, ridge = wine_quality(n_test=3)
    ...
    best_conf = gpres2.minL_conf[best_idx]
    print "best_conf : ", best_conf
    print gpres2.ypreds[best_idx]

    #plt.plot(gpres2.ypreds, 'ro')
    #plt.plot(gpr_gd.y_train[:200])
    #plt.show()

# run GPR_GD with workload data(Real data).
