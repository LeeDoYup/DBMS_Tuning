...
"""
- disabled stopwatch
- decresed number of parameters for run_lasso
"""
...
import gc
import pdb
from gp_tf import read_wine_quality # this function should be moved into util.py
from matplotlib import pyplot as plt

"""Get X,Y and execute lasso Path algorithm
   Output : Alpha(Lambda), Coef(Corresponding Coefficient)
"""
...
def run_lasso(include_polynomial_features=True):
    ...
    x_columnlabels = np.array(['fixed acidity','volatile acidity','critic acid','residual sugar',
			       'chlorides','free sulfur dioxide','total sulfur dioxide','density', 'pH', 
			       'sulphates','alcohol','dummy']) # 'dummy' for stdev_zero testing.
    y_columnlabels = np.array(['quality'])
    ...
    #--------------------------------
    # Filter out stdev_zero columns.
    #--------------------------------
    # Metrics are also filtered. 
    if y.shape[1] > 1:
    ...
    #----------------------------------------------------------
    # Transform categorical knob into one-hot encoded feature.
    # We don't have to use it right now. 
    # dbms object should be investigated later.
    #----------------------------------------------------------
    """
    ...
    # Write final ordering as a text file.
    ...
    with open("featured_knobs.txt", "w") as f:
        f.write("\n".join(final_ordering))

    # Plot result.
    lasso.plot_path(show=True)
