...
class KMeans_(object):
    ...
    def __init__(self, X, cluster_range, n_init=50):
        self.clusters_ = np.arange(*cluster_range)[2:]
        self.cluster_map_ = {}
...
# MOG using sklearn library.
class GaussianMixture_(object):
    
    def __init__(self, X, cluster_range, n_init=50):
        self.clusters_ = np.arange(*cluster_range)
        self.cluster_map_ = {}
        for k in self.clusters_:
            model = GaussianMixture_.fit_model(X, n_init)
            self.cluster_map_[k] = model.predict(X)
            #self.cluster_map_[k] = GaussianMixture_.get_model_attributes(model)

    # Fine proper the number of cluster using AIC. 
    # Return mog model.
    @staticmethod
...
# Spectral clustering using sklearn library.
class Spectral_clustering_(object):
    
    def __init__(self, X, cluster_range, n_init=50):
        self.clusters_ = np.arange(*cluster_range)
        self.cluster_map_ = {}
        for k in self.clusters_:
            model = Spectral_clustering_.fit_model(X, k, n_init)
            self.cluster_map_[k] = Spectral_clustering_.get_model_attributes(model)

    @staticmethod
    def fit_model(X, K, n_init=50):
        spectral_clustering = SpectralClustering(K, n_init=n_init)
        spectral_clustering.fit(X)
        return spectral_clustering

    # 미구현 부분 : 클러스터 대표 원소를 결정하여 return에 추가할 것.
    @staticmethod
    def get_model_attributes(model):
        #return model.cluster_centers_, model.labels_, model.inertia_
...
# Hierarchy clustering using sklearn libarary.
class Hierarchy_clustering_(object):
    
    def __init__(self, X, cluster_range, n_init=50):
        self.clusters_ = np.arange(*cluster_range)
        self.cluster_map_ = {}
        for k in self.clusters_:
            model = Hierarchy_clustering_.fit_model(X, k, n_init)
            knee = np.diff(model[::-1, 2], 2)
            K = knee.argmax() + 2
            self.cluster_map_[k] = hierarchy.fcluster(model, K, 'maxclust')

    @staticmethod
    def fit_model(X, K, n_init=50):
        hierarchy_clustering = hierarchy.linkage(X)
        return hierarchy_clustering
...
class GapStatistic(KSelection):
    ...
    def __init__(self, X, cluster_range, cluster_map, n_B=5):
...
# Used Simple Heuristic Algorithm[D. T. Pham, et. al., Selection of k in k-means clustering, 2005]
